...
def main(args):
    ...
    if args.mesh:
        n_points = 778
    n_points *= args.seq_length

    src_mask = torch.tensor([[[True] * n_points]]).cuda()
    ...
    model_pos = GraFormer(adj=adj.cuda(), hid_dim=args.dim_model, coords_dim=(2, 3), n_pts=n_points, num_layers=args.n_layer, n_head=args.n_head, dropout=args.dropout).cuda()
